...
describe('getVisiblePersonIds - comportement par défaut', () => {
  it('affiche uniquement "Mes proches" si aucun filtre actif', () => {
    ...
    // Personne sélectionnée + 4 proches
    expect(visible.size).toBe(5);
    expect(visible.has('1')).toBe(true);
    expect(visible.has('2')).toBe(true); // parent
    expect(visible.has('3')).toBe(true); // child
    expect(visible.has('4')).toBe(true); // sibling
    expect(visible.has('5')).toBe(true); // spouse
  });
